...
export default function AIChatbot() {
  ...
  const generateChatPDF = () => {
    ...
    try {
      ...
      // Extract session start time from sessionId (format: session_TIMESTAMP_RANDOM)
      const sessionTimestamp = parseInt(sessionId.split('_')[1], 10);
      const firstMessageTime = new Date(sessionTimestamp);
      ...
      // Helper to strip markdown for plain text (NO EMOJI)
      ...
      // Generate timestamps for each message distributed evenly across conversation duration
      ...
      const docDefinition: any = {
        ...
        header: (currentPage: number, pageCount: number) => {
          ...
          return {
            margin: [40, 20, 40, 20],
            columns: [
              { text: 'Gulfood 2026 Chat Transcript', style: 'headerText' },
              { text: `Page ${currentPage} of ${pageCount}`, style: 'headerText', alignment: 'right' }
            ]
          };
        },
        footer: (currentPage: number) => {
          ...
          return {
            margin: [40, 20, 40, 20],
            columns: [
              { text: 'Generated by Gulfood Event AI', style: 'footerText' },
              { text: now.toLocaleDateString(), style: 'footerText', alignment: 'right' }
            ]
          };
        },
        content: [] as any[],
      ...
      docDefinition.content.push(
        { text: 'Gulfood 2026', style: 'coverTitle' },
        { text: 'Chat Transcript', style: 'coverSubtitle' },
        { text: 'With: Faris (Your Event Guide)', style: 'coverInfo' },
        { text: `Conversation Started: ${firstMessageTime.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} at ${firstMessageTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`, style: 'coverInfo' },
        { text: `PDF Generated: ${now.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} at ${now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`, style: 'coverInfo' }
      );

      if (leadInfo.name && leadInfo.email) {
        docDefinition.content.push(
          { text: '', margin: [0, 20, 0, 0] },
          { text: `User: ${leadInfo.name}`, style: 'coverInfo' },
          { text: `Email: ${leadInfo.email}`, style: 'coverInfo' }
        );
      }

      docDefinition.content.push(
        { text: '', margin: [0, 20, 0, 0] },
        { text: `Total Messages: ${messages.length}`, style: 'coverInfo' },
        { text: `Conversation Duration: ${duration > 0 ? duration : '< 1'} mins`, style: 'coverInfo' },
      ...
      messages.forEach((message, idx) => {
        ...
        const timestamp = messageTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
          ...
          docDefinition.content.push({
            table: {
              ...
              body: [[
                {
                  stack: [
                    { text: `Faris • ${timestamp}`, style: 'messageTime', margin: [0, 0, 0, 5] },
                    { text: cleanContent, style: 'botMessage' }
                  ],
                  fillColor: '#F5F5F5',
                  margin: [8, 8, 8, 8]
                }
          ...
          docDefinition.content.push({
            table: {
              ...
              body: [[
                {
                  stack: [
                    { text: `${timestamp} • You`, style: 'messageTime', alignment: 'right', margin: [0, 0, 0, 5] },
                    { text: cleanContent, style: 'userMessage' }
                  ],
                  fillColor: '#FFFFFF',
                  margin: [8, 8, 8, 8]
                }
      ...
      docDefinition.content.push({ text: 'Conversation Summary', style: 'summaryTitle' });
      docDefinition.content.push({ text: `Messages from Faris: ${botMessageCount}`, style: 'summaryText' });
      docDefinition.content.push({ text: `Messages from User: ${userMessageCount}`, style: 'summaryText' });
      docDefinition.content.push({ text: '', margin: [0, 15, 0, 10] });
      docDefinition.content.push({ text: 'Session Information:', style: 'summaryText', bold: true });
      docDefinition.content.push({ text: `Session ID: ${sessionId}`, style: 'summaryText' });
      docDefinition.content.push({ text: `User Role: ${userRole || 'Not specified'}`, style: 'summaryText' });

      if (leadInfo.name && leadInfo.email) {
        docDefinition.content.push({ text: '', margin: [0, 15, 0, 10] });
        docDefinition.content.push({ text: 'Contact Information:', style: 'summaryText', bold: true });
        docDefinition.content.push({ text: `Name: ${leadInfo.name}`, style: 'summaryText' });
